...
export function ShareButtons({ brand, model, year, price }: { brand: string; model: string; year: number; price: number }) {
  const { showToast } = useToast();
  const [copied, setCopied] = useState(false);

  const shareText = `Lihat ${brand} ${model} (${year}) - Rp ${price.toLocaleString("id-ID")} di Rumapea Automotive!`;
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-stone-400 font-semibold uppercase tracking-wider mr-1">Share:</span>
      <button
        onClick={shareToWhatsApp}
        className="w-8 h-8 rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-600 hover:bg-emerald-100 transition-colors"
      ...
      <button
        ...
        className="w-8 h-8 rounded-full bg-sky-50 border border-sky-200 flex items-center justify-center text-sky-600 hover:bg-sky-100 transition-colors"
      ...
      <button
        onClick={copyLink}
        className={`w-8 h-8 rounded-full border flex items-center justify-center transition-colors ${copied ? "bg-emerald-100 border-emerald-300 text-emerald-700" : "bg-stone-50 border-stone-200 text-stone-500 hover:bg-stone-100"}`}
        title="Salin Link"
      >
        <Link2 size={14} />
      </button>
